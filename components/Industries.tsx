  ...
  return (
    <section id="industries" className="py-20 md:py-28 px-6 md:px-12 bg-[#F3F0E9] text-[#0A0A0A] border-b border-[#0A0A0A] relative paper-grain">
      <div className="max-w-7xl mx-auto space-y-16 md:space-y-24">
        ...
        {/* Grid Container with Even Column Spacing and No Lines */}
        <div className="pt-12 md:pt-16">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-12 md:gap-y-16 lg:gap-x-12">
            {industries.map((ind, idx) => (
              <motion.div
                key={ind.code}
                initial={{ opacity: 0, y: 16 }}
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.35, delay: idx * 0.06 }}
                className="flex flex-col justify-between py-2 relative"
              >
                <div>
                  <div className="flex justify-between items-center mb-5">
                    <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-mono text-[#0A0A0A]/40 uppercase tracking-wider">
                      SECTOR // {ind.num}
                    </span>
                  </div>

                  <h3 className="text-xl sm:text-2xl font-display uppercase tracking-tight mb-1 text-[#0A0A0A] leading-snug">
                    {ind.name}
                  </h3>
                  ...
                  <p className="text-xs sm:text-sm text-[#0A0A0A]/80 font-light leading-relaxed mb-6">
                    {ind.focus}
                  </p>
                </div>
                ...
                <div>
                  <div className="flex flex-wrap gap-1.5 pt-2">
                    {ind.stack.map((item, i) => (
                      <span
                        key={i}
                        className="text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 border border-[#0A0A0A]/20 bg-white/40 text-[#0A0A0A]"
                      >
                        {item}
                      </span>
                    ))}
